...
4. **Whole-body morph.** One smooth displacement field is applied to every mesh, retained and fitted alike: stature scaled to about 1.62 m, shoulders narrowed, thorax and waist narrowed, pelvis widened at the iliac crests, the skull scaled down about its centre, and the nasal bridge and projection reduced inside a smooth window around the nasal bones and cartilages. These are artistic parameter choices, not measurements of a female subject or evidence for a demographic facial profile. Lateral changes saturate beyond the torso's half-width so the arms translate with the shoulders and hips rather than being squeezed. The implementation samples the Jacobian and records a positive sampled minimum (currently 0.3784). This is not proof of positivity everywhere, correct joint mechanics, preserved muscle attachments or freedom from intersections. Applying a shared field moves structures together but does not validate their anatomical relationships, particularly at source replacements. Normals use the inverse-transpose of the local Jacobian.
...
| `biacromialWidth` | Combined lateral bounds of the scapulae, not acromion-to-acromion points | 0.2944 m |
| `biIliacWidth` | Combined lateral bounds of the ilia after fitting; original comparison uses complete hip bones | 0.2975 m |
...
All shape adjustments are artistic estimates: the stature factor 0.95, lateral height-dependent factors 0.91–1.10, thorax depth factor 0.96, head factor 0.95, nasal projection factor 0.52, pelvis and reproductive bounding-box fits, and the regenerated breast contour. The source data and transforms are traceable, but no declared female subject supplies these target proportions. Stature scaling combines with the head adjustment, so the final extent is not simply 95% of the initial height.

Women vary in body proportions, muscle mass and distribution; no single male-to-female scale factor represents that variation. A large MRI study reports differences associated with sex, age and body size, but it does not validate this model's chosen factors ([Janssen et al., 2000](https://pubmed.ncbi.nlm.nih.gov/10904038/)). SWR-517 still requires a declared reference build and anatomical point measurements, followed by attachment validation. Bone shape, muscle volume and soft-tissue distribution cannot be validated by silhouette alone.
